...
<div class="card-btn d-flex justify-content-between mt-2">
  <p style="color:black; font-size:1rem; font-weight:bold">3 Best Top Kost</p>
</div>
<div class="row match-height">
  @foreach ($kamar->sortByDesc(function($kamars) {
    return $kamars->reviews->avg('rating');
  })->take(3) as $kamars)
            ...
            <div class="row">
              <div class="col-lg-12"> 
                <div class="star-rating">
                  @for ($i = 1; $i <= 5; $i++)
                    @if ($i <= $kamars->reviews->avg('rating'))
                    <span class="fa fa-star star-rating"></span>
                    @else
                    <span class="fa fa-star-o" ></span>
                    @endif
                  @endfor          
                  @if($kamars->reviews->avg('rating')==0)
                  <span>.  0 / 5</span>
                  @else
                  <span>. {{$kamars->reviews->avg('rating')}}  / 5</span>
                  @endif
                </div>
              </div>
            </div>
